...
export default function CongratulationsContent() {
  ...
  const title = isPasswordResetSuccess
    ? 'Password Changed Successfully!'
    : 'Account Created Successfully!';

  const description = isPasswordResetSuccess
  ...
  const buttonText = isPasswordResetSuccess
    ? 'Continue to Login'
    : 'Go to Home';

  const buttonHref = isPasswordResetSuccess ? '/login' : '/';
